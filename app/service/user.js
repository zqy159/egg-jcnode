...
class UserService extends Service {
  ...
  async getUsersByPhone(phone) {
    ...
    return await this.ctx.modelmysql.User.findOne({
      where: {
        phone,
      },
    });
  ...
  async createUsers({ phone, password, avatar, nickname }) {
    if (!phone) {
      return false;
    }
    return await this.ctx.modelmysql.User.create({
      phone,
      password,
      avatar,
      nickname,
  ...
  async updatePassword({ phone, password }) {
    ...
    return await this.ctx.modelmysql.User.update({ password }, {
      where: {
        phone,
      },
    });
  ...
  async validUserByPhoneAndPwd({ phone, password }) {
    if (!phone || !password) {
      return false;
    }
